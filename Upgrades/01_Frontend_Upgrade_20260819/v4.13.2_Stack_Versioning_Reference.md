# Post v4.13.2 Upgrade Stack Versioning Reference — gene.iobio Local Stack  
*Flat version reference — scannable lookup only*  
*Last updated: H20260819 post v4.13.2 Upgrade*  

**v4.13.2 Upgrade to build procedure:** See `01_Stack_Build_Steps.md`  
**For previous history:** See `02_Session_Handoff_Log.md`  

---  

## Host / Windows Layer - v4.13.2 posed no changes  
...
## WSL Configuration - v4.13.2 posed no changes  
...
## Windows Power Management - v4.13.2 posed no changes 
| Item | Value |  
|---|---|
...
## Podman Configuration - v4.13.2 posed no changes  
| Item | Value |  
|---|---|
| `Podman` | `5.7.0` |  
...
## Core WSL Packages - v4.13.2 posed no changes  
...
## Application Layer — Current Production Stack - v4.13.2 posed no changes  
...
## Frontend Layer  
| Component | Version | Location | v4.13.2 Notes |  
|---|---|---|---|
| `gene.iobio` | `v4.13.2` | `~/gene.iobio/` | `commit 1854f59` |  
| `NVM` | `0.39.0` | `~/.nvm/` | No impact |   
| `Node.js` | `v20.20.2` | via `nvm` | ✅ Verified |  
| `npm` | `10.8.2` | via `nvm` | ✅ Verified |  
| `webpack` | `3.12.0` | `npm` dep | build output `client/dist/build.js` `30.5MB` | No impact |  
| `serve` | `installed globally` | via `npm` `npm install -g serve` | No impact |  

---

## Phase 8 Option A Modifications (`CSV`/`VCF` Export)  
| File | v4.13.1 Line | v4.13.2 Line | Edit | v4.13.2 Status | 
|---|:---:|:---:|:---:|---|    
| `client/app/components/viz/FlaggedVariantsCard.vue` | `843` | `836` | gate → `cohortModel.isLoaded` | ✅ Present |   
| `client/app/components/viz/Navigation.vue` | `952` | `947` | gate → `cohortModel.isLoaded` | ✅ Present |   
| `client/app/models/CohortModel.js` | `2903` | `2904` | `reject(error)` → `resolve()` | ✅ Applied |  

CohortModel.js backed up as `~/CohortModel.js.bak`. Build: 26 seconds, clean.  

---

## Phase 8 Option B Configuration (LAN Network Serving/WSL IP Persistence)  
| Item | Value | v4.13.2 Status | 
|---|---|---|    
| `config.json` `backend.origin` | `http://192.168.1.240:9001` | ✅ Applied |  
| `config.json` `backend_map.default` | `http://192.168.1.240:9001` | ✅ Applied |  
| `Firewall rule` — `frontend` | `port 3000, Inbound, Allow, Enabled=True` | No impact |   
| `Firewall rule` — `backend` | `port 9001, Inbound, Allow, Enabled=True` | No impact |    
| `netsh portproxy` — `frontend` | `0.0.0.0:3000 → 172.18.60.70:3000` | No impact |    
| `netsh portproxy` — `backend` | `0.0.0.0:9001 → 172.18.60.70:9001` | No impact |    
| Startup script | `~/start_network_frontend.sh` | No impact |    
| LAN access `URL` | `http://192.168.1.240:3000` | No impact |    
...
## gru_data 2.0.0 Contents - v4.13.2 posed no changes  
| Item | Path | Status |  
|---|---|---|  
...
## Network / Share Architecture - v4.13.2 posed no changes  
...
`vault` (`192.168.1.52`) share architecture intentionally minimal.  
There is intentionally no share to the local `D:` SSK External 1Tb USB drive.
---
...
| `ubuntu_full_stack_v4_13_2_FINAL_20260819.tar` | 'FINAL` - `post v4.13.2 Upgrade` | '17,078,760 KB` | `16.288 GB` | `9301C452A1C6DBE7535915AD84B98FF8` |  
...
| `gene_iobio_frontend_v4_13_2_FINAL_20260819.tar` | 'FINAL` - `post v4.13.2 Upgrade` | `310,720 KB` | `0.296 GB` | `C47464D3B71CFC85E81F9B5C7A43FDC5` | ✅ | ✅ |   
...
Validated: End-to-end with production WGS data (`GRCh37/38`).  
...
≡≡≡≡≡ END: Upgrades/01_Frontend_Upgrade_20260819/v4.13.2_Stack_Versioning_Reference.md ≡≡≡≡≡
